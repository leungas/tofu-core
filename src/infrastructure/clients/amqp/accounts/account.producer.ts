...
export class AccountServiceProducer {
     ...
     * @type {Logger}
     */
    private readonly logger: Logger = new Logger('producer[account]', {timestamp: true});
    ...
    async registerCapabilities(items: any[]) {
        ...
        this.logger.debug(`registerCapability(): $items = ${JSON.stringify(items)}`);
        const exchange = this.config.get('exchange.exchanges.accounts.name');
        this.logger.debug(`registerCapability(): $exchange = ${JSON.stringify(exchange)}`);
        const routing = this.config.get('exchange.exchanges.accounts.capability.register');
        this.logger.debug(`registerCapability(): $routing = ${JSON.stringify(routing)}`);
