...
export class AccountModelRepository {
     ...
     * @type {Logger}
     */
    private readonly logger: Logger = new Logger('repo[account]', {timestamp: true});
    ...
    async create(entity: Workspace, admin: User) {
        ...
        const result = await this.client.transaction(async (m) => {
            ...
            if (!user) 
                user = await m.save(Object.assign(new UserModel(), admin));
            const workspace = await m.save(Object.assign(new WorkspaceModel(), entity, {account: account, admin: user}));
            ...
            const teams = roles.map((r: SystemTeamModel) => {
                ...
                if (r.autoAssign) {
                    const member = Object.assign(new MemberModel(), {team: team, user: user});
